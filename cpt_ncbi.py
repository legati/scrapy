...
class GenNcbiSpider(scrapy.Spider):
  ...
  def parse(self,response):
    ...
    next_page =  response.xpath('//div[@class = "other-issues"]//li[@class = "next-issue"]/a[@class = "navlink"]/@href').extract_first()
  ...
  def parse_site_data (self, response):
    ...
    if bool(div_tags) != False: # *Corresponsence to, Corresponding author etc cases
      ...
      auth = response.xpath('//div[@class="half_rhythm"]/div[@class="contrib-group fm-author"]/sup[text() = "*"]/preceding-sibling::a[1]/text()').extract_first()
      if auth:
        auth = auth.strip('* :')
        auth = auth.split(",")[0]
        auth = scrtools.get_clean_author (auth)
